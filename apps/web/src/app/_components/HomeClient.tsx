...
import { assetUrl, useApi, type ActivityEntry, type Category, type FeedResponse } from "@/lib/api";
import { formatDate } from "@/lib/format";

const FEATURED_LIMIT = 8;
const FEATURED_SLIDES = 5;
const BROWSE_LIMIT = 24;
const FEATURED_SEED = "home";

/**
 * 首页数据由 Server Component 取好后经 fallback 注入 SWR：
 * HTML 到达时分类栏、精选、继续观看、逛逛列表都已就位，封面可以立即并发下载，
 * 不再需要「HTML → JS → 四个 JSON → 图片」这样两轮串行。
 */
export function HomeClient({ browseSeed, fallback }: { browseSeed: string; fallback: Record<string, unknown> }) {
  return (
    <SwrFallback data={fallback}>
      <HomeView initialBrowseSeed={browseSeed} />
    </SwrFallback>
  );
}

function HomeView({ initialBrowseSeed }: { initialBrowseSeed: string }) {
  const [featuredIndex, setFeaturedIndex] = useState(0);
  // seed 由服务端下发一次：既避免 Date.now() 造成的水合前后不一致，
  // 也保证 SSR 与 SWR fallback 用的是同一个 key
  const [browseSeed, setBrowseSeed] = useState(initialBrowseSeed);

  const { data: categoryData } = useApi<{ categories: Category[] }>("/categories");
  const { data: continueData } = useApi<{ entries: ActivityEntry[] }>("/me/continue-watching?limit=4");
  const { data: featuredData, isLoading: featuredLoading } = useApi<FeedResponse>(
    `/feeds/home?seed=${encodeURIComponent(FEATURED_SEED)}&limit=${FEATURED_LIMIT}`
  );
  ...
  const continueWatching = continueData?.entries ?? [];
