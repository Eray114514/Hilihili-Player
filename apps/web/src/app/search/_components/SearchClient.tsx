...
/**
 * 搜索结果由 Server Component 取好后经 fallback 注入 SWR。
 *
 * 搜索框是原生 GET 表单提交（整页导航），所以首屏结果页可以直接带上结果与封面 URL。
 * query 用 key remount 交给 SearchResults，切换关键词时重置“加载更多”的本地状态。
 */
export function SearchClient({ initialQuery, fallback }: { initialQuery: string; fallback: Record<string, unknown> }) {
  return (
    <SwrFallback data={fallback}>
      <SearchResults key={initialQuery} query={initialQuery} />
    </SwrFallback>
  );
}
