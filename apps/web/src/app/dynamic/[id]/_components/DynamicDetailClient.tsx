...
/**
 * 动态详情由 Server Component 取好后经 fallback 注入 SWR：
 * 正文与图片 URL 随 HTML 一起到达，图片立即开始下载。
 */
export function DynamicDetailClient({ id, fallback }: { id: string; fallback: Record<string, unknown> }) {
  return (
...
function DynamicDetailView({ id }: { id: string }) {
  const { data: detail, error } = useApi<ItemDetail>(`/items/${id}`);
