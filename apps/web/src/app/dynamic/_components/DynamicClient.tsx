...
/**
 * 动态数据由 Server Component 取好后经 fallback 注入 SWR：
 * 首屏 HTML 直接带上第一页动态与封面 URL，图片可以立刻开始下载。
 * seed 由服务端下发，保证 SSR 与 hydration 用的是同一个 SWR key。
 */
export function DynamicClient({ initialSeed, fallback }: { initialSeed: string; fallback: Record<string, unknown> }) {
  return (
    <SwrFallback data={fallback}>
      <DynamicView initialSeed={initialSeed} />
    </SwrFallback>
  );
}

function DynamicView({ initialSeed }: { initialSeed: string }) {
