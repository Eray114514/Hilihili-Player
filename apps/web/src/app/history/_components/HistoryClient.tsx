...
/**
 * 活动数据由 Server Component 取好后经 fallback 注入 SWR；
 * tab 初值来自 URL 的 ?tab=，切换时本地 state 立即生效并同步回 URL。
 */
export function HistoryClient({ initialTab, fallback }: { initialTab: ActivityTab; fallback: Record<string, unknown> }) {
  return (
    <SwrFallback data={fallback}>
      <HistoryView initialTab={initialTab} />
    </SwrFallback>
  );
}

function HistoryView({ initialTab }: { initialTab: ActivityTab }) {
    ...
    <AppShell>
      ...
      <div className="mb-5 flex gap-2 overflow-x-auto pb-1">
        {tabs.map((item) => {
          ...
          return <button key={item.id} className={`secondary-button shrink-0 ${tab === item.id ? "!bg-[var(--accent-soft)] !text-[var(--accent)]" : ""}`} onClick={() => selectTab(item.id)}><Icon size={16} />{item.label}</button>;
        })}
      </div>
