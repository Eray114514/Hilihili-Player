...
// 注意：dynamic(..., { ssr: false }) 不允许出现在 Server Component 里，
// 所以这个 import 必须留在客户端组件这一侧。
...
/**
 * 详情数据由 Server Component 取好后经 fallback 注入 SWR。
 * 这样首屏 HTML 里已经带了标题/分P/相关推荐，图片与字幕 URL 可以立刻并发请求，
 * 不需要再等一轮「HTML → JS → JSON」才能知道要下什么。
 */
export function WatchClient({ itemId, fallback }: { itemId: string; fallback: Record<string, unknown> }) {
  return (
    <SwrFallback data={fallback}>
      <WatchView itemId={itemId} />
    </SwrFallback>
  );
}

function WatchView({ itemId }: { itemId: string }) {
  // 关 focus revalidate，避免用户看视频时背景刷新重置本地 state
  const { data: detail, error, mutate: mutateDetail } = useApi<ItemDetail>(`/items/${itemId}`, { revalidateOnFocus: false });
...
function formatDate(value: string | null) {
  ...
  // 固定时区：这段会在服务端预渲染一次、客户端 hydration 一次，
  // 用运行环境的本地时区会让两边的字符串不一致（Node 与浏览器的 TZ/ICU 不同）。
  return date && Number.isFinite(date.getTime())
    ? date.toLocaleString("zh-CN", { year: "numeric", month: "long", day: "numeric", timeZone: "Asia/Shanghai" })
    : "日期未知";
}
