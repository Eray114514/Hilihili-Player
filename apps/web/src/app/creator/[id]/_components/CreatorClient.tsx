...
/**
 * UP 主页数据由 Server Component 取好后经 fallback 注入 SWR：
 * 头像、横幅与第一页投稿的 URL 随 HTML 一起到达。
 */
export function CreatorClient({ id, fallback }: { id: string; fallback: Record<string, unknown> }) {
  return (
...
function CreatorView({ id }: { id: string }) {
  ...
  const itemsKey = `/creators/${id}/items?limit=24${itemsSuffix}`;
  ...
  const { data: profile, error: profileError, mutate: mutateProfile } = useApi<CreatorDetail>(`/creators/${id}`);
  ...
  async function toggleFollow() {
    ...
    setBusy("follow");
    try {
      await putJson(`/creators/${id}/follow`, { followed: next });
  ...
  async function toggleBlock() {
    ...
    setBusy("block");
    try {
      await putJson(`/creators/${id}/blacklist`, { blacklisted: next });
  ...
  return (
    <AppShell wide>
      ...
      <section className="mt-9">
        ...
        {hasMore ? <div className="flex justify-center pt-10"><button disabled={loadingMore} onClick={() => { const suffix = kind === "all" ? "" : `&kind=${kind}`; setLoadingMore(true); void getJson<CreatorItemsResponse>(`/creators/${id}/items?limit=24&offset=${items.length}${suffix}`).then((response) => { // 把追加页累积进 SWR 缓存（不触发 revalidate，避免重新请求第一页）。
