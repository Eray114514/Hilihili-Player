...
/**
 * 消息列表由 Server Component 取好后经 fallback 注入 SWR。
 * “标记全部已读”仍是进入页面后的客户端副作用（POST /me/messages:read），保持不动。
 */
export function MessagesClient({ fallback }: { fallback: Record<string, unknown> }) {
  return (
...
function MessageSkeleton() { return <div className="space-y-3 skeleton-shimmer">{Array.from({ length: 5 }, (_, index) => <div key={index} className="flex gap-4 rounded-2xl border border-white/6 bg-white/[0.025] p-3"><div className="aspect-video w-52 rounded-xl bg-white/6" /><div className="flex-1 py-2"><div className="h-4 w-32 rounded bg-white/6" /><div className="mt-4 h-5 w-2/3 rounded bg-white/6" /></div></div>)}</div>; }