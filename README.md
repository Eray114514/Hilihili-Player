...
推荐在标签所属范围的 `info.json` 中写入 `tags`：分区的 `info.json` 用于全分区默认标签，UP 主的 `info.json` 用于该 UP 的默认标签，内容目录的 `info.json`（或平铺文件的同名 `.info.json`）用于单条内容。扫描时按“分区 → UP 主 → 内容”合并，后层可移除继承标签：
...
`"tags": ["数码"]` 是只添加标签的简写。根目录 `_tags.json` 仍会被读取，以兼容旧媒体库；但它只在对应层级没有新 `tags` 字段时兜底，新整理内容应优先使用就近元数据。
